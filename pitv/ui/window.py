...
class PitvApp:
    ...
    def _build_layout(self) -> None:
        ...
        for label, desc in (
            ("↑   ↓", "Move"),
            ("Enter", "Select, watch, or cancel scan"),
            ("Enter", "Menu / Back to TV while watching"),
            ("Esc", "Quit"),
        ):
    ...
    def _quit(self) -> None:
        ...
        self._closing = True
        if self.controller.busy:
            # Let the scan worker terminate its dvbv5-scan subprocess so we
            # don't leak a process that keeps the DVB adapter busy.
            self.controller.cancel_busy()
